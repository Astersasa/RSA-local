...
clear_message=te.read()
te.close()
#change the text to the intger based on ascii code
cm_list=strc(clear_message)
#encrypt the clear_message
if os.path.exists("ciper_word.txt"):
    os.remove("ciper_word.txt")
if os.path.exists("ciper_list.txt"):
    os.remove("ciper_list.txt")
for i in cm_list:
    ciper_message,l=encrypt(i,e,n)
# os.remove("password.txt")
    te=open('ciper_word.txt','a')
    te.write(ciper_message)
    te.write('\n')
    te.close()
    te=open('ciper_list.txt','a')
    for i in l:
        te.write(str(i))
        te.write(" ")
    te.write('\n')
